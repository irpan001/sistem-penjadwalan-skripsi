...
session_start();
    ...
    // data polot kelas
    $sql_kelas =  "SELECT a.kode_kelas, a.matakuliah, a.dosen, b.jml_sks FROM plot_kelas as a, matakuliah as b where a.matakuliah = b.kode_matkul AND a.sesi = 3;";
    $ex = $db_con->query($sql_kelas) or die (mysql_error());
    $no = 0;
    while($data=$ex->fetch(PDO::FETCH_OBJ)){
    $this->kode_kelas[$no] = $data->kode_kelas;
    $this->kode_matkul[$no] = $data->matakuliah;
    ...
    // data jam
    $sql_jam =  "SELECT * FROM jam WHERE keterangan = 'W'";
    ...
    $no++;
        ...
        $this->RowsKelas = count($this->kode_kelas);
        ...
        // $c = count($this->kode_kelas);
        // return $this->b;
        // return $this->RowsKelas;
    
            // for ($i=0; $i < 10 ; $i++) { 
        $sql = $db_con->query("TRUNCATE TABLE data_acak");
                for ($j=0; $j < $this->RowsKelas  ; $j++) { 
                
                $this->data_individu[$j][0] = $this->kode_kelas[$j];
        ...
        $sql = $db_con->query("INSERT INTO data_acak VALUES ($a,$b,$c,$d)");
...
// end class
 ...
 echo '<META HTTP-EQUIV="Refresh" Content="0; URL=index.php?page=83&&fit='.$fitness.'">';
