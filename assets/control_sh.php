...
session_start();
    ...
    // data polot kelas
    $sql_kelas =  "SELECT a.kode_semhas, a.penelitian, a.dosen, b.jml_sks FROM plot_semhas as a, penelitian as b where a.penelitian = b.kode_penelitian ;";
    $ex = $db_con->query($sql_kelas) or die (mysql_error());
    $no = 0;
    while($data=$ex->fetch(PDO::FETCH_OBJ)){
    $this->kode_semhas[$no] = $data->kode_semhas;
    ...
    $no++;
        ...
        $this->RowsKelas = count($this->kode_semhas);
        ...
        // $c = count($this->kode_semhas);
        // return $this->b;
        // return $this->RowsKelas;
    
            // for ($i=0; $i < 10 ; $i++) { 
        $sql = $db_con->query("TRUNCATE TABLE data_acak_semhas");
                for ($j=0; $j < $this->RowsKelas  ; $j++) { 
                
                $this->data_individu[$j][0] = $this->kode_semhas[$j];
        ...
        $sql = $db_con->query("INSERT INTO data_acak_semhas VALUES ($a,$b,$c,$d)");
...
// end class
    ...
    $t->Data();
              ...
              include '../vendors/koneksi.php';
                                        $sql = "SELECT * FROM data_acak_semhas";
