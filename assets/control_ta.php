...
session_start();
    ...
    // data polot kelas
    $sql_kelas =  "SELECT a.kode_akhir, a.penelitian, a.dosen, b.jml_sks FROM plot_akhir as a, penelitian as b where a.penelitian = b.kode_penelitian ;";
    $ex = $db_con->query($sql_kelas) or die (mysql_error());
    $no = 0;
    while($data=$ex->fetch(PDO::FETCH_OBJ)){
    $this->kode_akhir[$no] = $data->kode_akhir;
    ...
    $no++;
        ...
        $this->RowsKelas = count($this->kode_akhir);
        ...
        // $c = count($this->kode_akhir);
        // return $this->b;
        // return $this->RowsKelas;
    
            // for ($i=0; $i < 10 ; $i++) { 
        $sql = $db_con->query("TRUNCATE TABLE data_acak_akhir");
                for ($j=0; $j < $this->RowsKelas  ; $j++) { 
                
                $this->data_individu[$j][0] = $this->kode_akhir[$j];
        ...
        $sql = $db_con->query("INSERT INTO data_acak_akhir VALUES ($a,$b,$c,$d)");
...
// end class
    ...
    $t->Data();
              ...
              include '../vendors/koneksi.php';
                                        $sql = "SELECT * FROM data_acak_akhir";
